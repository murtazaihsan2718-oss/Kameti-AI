  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (attempt < MAX_RETRIES) {
        ...
        const jitter = Math.floor(Math.random() * (baseDelay * 0.3));
        const totalDelay = baseDelay + jitter;
        console.warn(`[Server Backend] Exception occurred: ${err.message}. Retrying attempt ${attempt + 2}/${MAX_RETRIES + 1} in ${totalDelay}ms...`);
        await sleep(totalDelay);
      } else {
...
async function handleTextToSpeech(req, res) {
  ...
  req.on('end', async () => {
    try {
      ...
      console.log(`[Server Backend] Synthesizing TTS (${chunks.length} chunks, lang: ${targetLang}): "${textToSpeak.slice(0, 40)}..."`);
      const buffers = [];
